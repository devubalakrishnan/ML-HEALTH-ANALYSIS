...
class ECG:
		...
		for x,y in enumerate(Leads[:len(Leads)-1]):
			...
			for contour in contours:
				if contour.shape in contours_shape:
					final_df = resize(contour, (255, 2))
			if (x+1)%3==0:
				ax4[x_counter][y_counter].invert_yaxis()
				ax4[x_counter][y_counter].plot(final_df[:, 1], final_df[:, 0],linewidth=1,color='black')
			...
			lead_no=x
			scaler = MinMaxScaler()
			fit_transform_data = scaler.fit_transform(final_df)
	...
	def DimensionalReduciton(self,test_final):
		...
		pca_loaded_model = joblib.load(str(BASE_DIR)+'/ml/PCA_ECG (1).pkl')
